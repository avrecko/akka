...
import akka.remote.rcl.arquillian.rcl.arquillian.FooPing;
...
public class RclTests extends RemoteTestCase {
    ...
    private final ActorRef echoActor = Actors.remote().actorFor("echo-actor", "localhost", 4273);
    private final ActorRef msgOrderActor = Actors.remote().actorFor("message-order-actor", "localhost", 4273);
    ...
    public void testMessageOrder() {
        for (int i = 0; i <= Settings.MSG_ORDER_TEST_REPEAT_LOOP; i++) {
            msgOrderActor.tell(new Foo()); // we just send something that is not on the classpath
